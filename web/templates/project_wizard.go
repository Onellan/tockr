...
func renderProjectWizardDetails(w io.Writer, user *NavUser, selectors *SelectorData, draft domain.ProjectCreateDraft) {
	...
	renderSelect(w, "Customer"+tipHTML("The client this project is billed to. Determines the billing unit and default billing contact."), "customer_id", optionList(selectors, "customer"), project.CustomerID, true, "Select a customer", nil)
	_, _ = fmt.Fprintf(w, `<label>Name %s<input name="name" value="%s" required></label>`, tipHTML("Project display name shown in timesheets, reports and invoices."), esc(project.Name))
...
func renderProjectWizardWorkstreams(w io.Writer, user *NavUser, selectors *SelectorData, draft domain.ProjectCreateDraft) {
	...
	renderSelect(w, "Existing workstream", "workstream_id", optionList(selectors, "workstream"), 0, false, "Select a workstream", nil)
	...
	_, _ = fmt.Fprintf(w, `<div class="section-spacer"></div><form method="post" action="/projects/create" class="form-grid"><input type="hidden" name="csrf" value="%s"><input type="hidden" name="step" value="workstreams"><label>Name<input name="new_name" required></label><label>Workstream ID<input name="new_code" placeholder="auto-generated if blank"></label><label>Budget split<input name="new_budget" value="0"></label><label class="wide">Description<textarea name="new_description"></textarea></label><div class="form-actions"><button class="primary" name="action" value="add-new">Create and add workstream</button></div></form></div>`, esc(user.CSRF))
...
func renderProjectWizardActivities(w io.Writer, user *NavUser, selectors *SelectorData, draft domain.ProjectCreateDraft) {
	...
	_, _ = fmt.Fprintf(w, `<div class="section-spacer"></div><form method="post" action="/projects/create" class="form-grid"><input type="hidden" name="csrf" value="%s"><input type="hidden" name="step" value="activities"><label>Deliverable<input name="new_name" required></label><label>Deliverable ID<input name="new_number" placeholder="auto-generated if blank"></label><label class="wide">Comment<textarea name="new_comment"></textarea></label><label class="check"><input type="checkbox" name="new_visible" checked> Visible</label><label class="check"><input type="checkbox" name="new_billable" checked> Billable</label><div class="form-actions"><button class="primary" name="action" value="add-new">Create and add deliverable</button></div></form></div>`, esc(user.CSRF))
...
func renderProjectWizardUsers(w io.Writer, user *NavUser, selectors *SelectorData, draft domain.ProjectCreateDraft) {
	_, _ = fmt.Fprint(w, `<section class="two-col section-spacer">`)
	_, _ = fmt.Fprintf(w, `<div class="panel form-panel"><div class="panel-head"><div><h2>Step 4 · Assign users</h2><p>Choose who should be able to work on the project and optionally set their role.</p></div></div><form method="post" action="/projects/create" class="form-grid"><input type="hidden" name="csrf" value="%s"><input type="hidden" name="step" value="users"><label class="wide">Users<select name="user_id" multiple size="10">`, esc(user.CSRF))
...
func renderGlobalActivitySelect(w io.Writer, selectors *SelectorData) {
	_, _ = fmt.Fprint(w, `<label>Existing deliverable<select name="activity_id"><option value="">Select a global deliverable</option>`)
