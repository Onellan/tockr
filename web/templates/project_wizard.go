...
func renderProjectWizardSteps(w io.Writer, current string) {
	steps := []struct {
		...
		{ID: "users", Label: "Assign team members"},
	}
...
func renderProjectWizardSummary(w io.Writer, selectors *SelectorData, draft domain.ProjectCreateDraft) {
	client := label(selectors.CustomerLabels, draft.Project.CustomerID)
	if client == "" {
		client = "Not selected"
	}
	...
	_, _ = fmt.Fprintf(w, `<section class="panel wizard-summary"><div class="panel-head"><div><h2>Project setup preview</h2><p>Nothing is saved until you click Create project.</p></div></div><div class="summary-list"><div><span class="field-hint">Project</span><strong>%s</strong></div><div><span class="field-hint">Client</span><strong>%s</strong></div><div><span class="field-hint">Workstreams</span><strong>%d</strong></div><div><span class="field-hint">Deliverables</span><strong>%d</strong></div><div><span class="field-hint">Team members</span><strong>%d</strong></div></div></section>`, esc(projectName), esc(client), len(draft.Workstreams), len(draft.Activities), len(draft.Members))
}
...
func renderProjectWizardDetails(w io.Writer, user *NavUser, selectors *SelectorData, draft domain.ProjectCreateDraft) {
	...
	renderSelect(w, requiredLabel("Client")+tipHTML("The client this project is billed to. Determines the billing unit and default billing contact."), "customer_id", optionList(selectors, "customer"), project.CustomerID, true, "Select a client", nil)
	...
	_, _ = fmt.Fprintf(w, `<label>Estimate hours %s<input name="estimate_hours" value="%d"></label>`, tipHTML("Total hours budgeted. Tockr tracks effort against this estimate in the project dashboard."), project.EstimateSeconds/3600)
...
func renderProjectWizardActivities(w io.Writer, user *NavUser, selectors *SelectorData, draft domain.ProjectCreateDraft) {
	_, _ = fmt.Fprint(w, `<section class="two-col section-spacer">`)
	_, _ = fmt.Fprintf(w, `<div class="panel form-panel"><div class="panel-head"><div><h2>Step 3 · Deliverables</h2><p>Select reusable global deliverables or define project-specific ones inline.</p></div></div><div class="info-callout"><strong>Note:</strong> Deliverables apply to the whole project and are shared across workstreams.</div><form method="post" action="/projects/create" class="form-grid section-spacer"><input type="hidden" name="csrf" value="%s"><input type="hidden" name="step" value="activities">`, esc(user.CSRF))
...
func renderProjectWizardUsers(w io.Writer, user *NavUser, selectors *SelectorData, draft domain.ProjectCreateDraft) {
	_, _ = fmt.Fprint(w, `<section class="two-col section-spacer">`)
	_, _ = fmt.Fprintf(w, `<div class="panel form-panel"><div class="panel-head"><div><h2>Step 4 · Assign team members</h2><p>Choose who should be able to work on the project and optionally set their role.</p></div></div><form method="post" action="/projects/create" class="form-grid"><input type="hidden" name="csrf" value="%s"><input type="hidden" name="step" value="users"><label class="wide">%s<select name="user_id" multiple size="10">`, esc(user.CSRF), requiredLabel("Team members"))
	for _, option := range selectors.Users {
		_, _ = fmt.Fprintf(w, `<option value="%d">%s</option>`, option.Value, esc(option.Label))
	}
	_, _ = fmt.Fprint(w, `</select></label><label>Role<select name="role"><option value="member">Member</option><option value="manager">Manager</option></select></label><div class="form-actions"><button class="primary" name="action" value="add-users">Add selected team members</button></div></form></div>`)
	_, _ = fmt.Fprint(w, `<div class="panel form-panel"><div class="panel-head"><div><h2>Selected team members</h2><p>These assignments are created when you finish the workflow.</p></div></div>`)
	if len(draft.Members) == 0 {
		_, _ = fmt.Fprint(w, `<div class="empty-state compact"><strong>No team members assigned yet</strong><span>You can finish without assigning team members, or add them now.</span></div>`)
	} else {
		_, _ = fmt.Fprint(w, `<div class="draft-chip-list">`)
