...
func (s *Store) scanUser(ctx context.Context, where string, args ...any) (*domain.User, error) {
	q := `SELECT id, organization_id, email, username, display_name, password_hash, timezone, enabled, totp_secret, totp_enabled, email_otp_enabled, created_at, last_login_at FROM users ` + where // #nosec G202
...
func (s *Store) ListOrganizationWorkspaces(ctx context.Context, organizationID int64, term string) ([]domain.WorkspaceSummary, error) {
	...
	rows, err := s.db.QueryContext(ctx, `SELECT w.id, w.organization_id, w.name, w.slug, w.description, w.default_currency, w.timezone, w.archived, w.created_at,
		...
		FROM workspaces w `+where+` ORDER BY w.name`, args...) // #nosec G202
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) ListCustomers(ctx context.Context, access domain.AccessContext, term string, page, size int) ([]domain.Customer, domain.Page, error) {
	...
	rows, err := s.db.QueryContext(ctx, `SELECT id, workspace_id, name, number, company, contact, email, currency, timezone, visible, billable, comment, legacy_json, created_at FROM customers `+where+` ORDER BY name LIMIT ? OFFSET ?`, append(args, size, (page-1)*size)...) // #nosec G202
	if err != nil {
		return nil, domain.Page{}, err
	}
	defer rows.Close()
...
func (s *Store) ListProjects(ctx context.Context, access domain.AccessContext, customerID int64, term string, page, size int) ([]domain.Project, domain.Page, error) {
	...
	rows, err := s.db.QueryContext(ctx, `SELECT id, workspace_id, customer_id, name, number, order_number, visible, private, billable, estimate_seconds, budget_cents, budget_alert_percent, comment, legacy_json, created_at FROM projects `+where+` ORDER BY name LIMIT ? OFFSET ?`, append(args, size, (page-1)*size)...) // #nosec G202
	if err != nil {
		return nil, domain.Page{}, err
	}
	defer rows.Close()
...
func (s *Store) ListActivities(ctx context.Context, access domain.AccessContext, projectID int64, term string, page, size int) ([]domain.Activity, domain.Page, error) {
	...
	rows, err := s.db.QueryContext(ctx, `SELECT id, workspace_id, project_id, name, number, visible, billable, comment, legacy_json, created_at FROM activities `+where+` ORDER BY name LIMIT ? OFFSET ?`, append(args, size, (page-1)*size)...) // #nosec G202
	if err != nil {
		return nil, domain.Page{}, err
	}
	defer rows.Close()
...
func (s *Store) ListTasks(ctx context.Context, access domain.AccessContext, projectID int64, term string, page, size int) ([]domain.Task, domain.Page, error) {
	...
	rows, err := s.db.QueryContext(ctx, `SELECT id, workspace_id, project_id, name, number, visible, billable, estimate_seconds, COALESCE(archived,0), created_at FROM tasks `+where+` ORDER BY name LIMIT ? OFFSET ?`, append(args, size, (page-1)*size)...) // #nosec G202
	if err != nil {
		return nil, domain.Page{}, err
	}
	defer rows.Close()
